  ...
  methods: {
    ...
    submit: function () {
      ...
      this.$http.post(`${window.noPointHost}/api/judge`, sendPackage).then(res => {
        this.isInfo = true
        if (res.body.code === 0) {
          this.submitInfo = '成功提交！'
          this.solutionId = res.body.data.solution_id
        } else {
          this.submitInfo = `${res.body.message}(${res.body.code})`
        }
      }, err => {
        this.isInfo = true
        if (err.body.code === 401) {
          this.submitInfo = '请您登录！'
        } else {
          this.submitInfo = `未知错误！(${err.body})`
        }
      })
    },
    setLan: function (Lan) {
