...
export default {
  ...
  methods: {
    ...
    ptpong: function () {
      ...
      this.princeton.rmt = []
      // console.log('清除了定时器')
      this.princeton.rmt.push = setTimeout(() => {
        console.log('失恋了')
        console.log(this.princeton)
    ...
    botalk: function (str) {
      this.$http.get(this.bot.url + '?key=' + this.bot.key + '&info=' + str + '&userid=' + this.bot.id)
        .then((res) => {
          var jsn = res.data
          var ret = jsn.text
    ...
    formMessage: function (arr) {
      ...
      const n = `<span class="user-${color}">${admin ? '[管]' : ''}${user}</span>: ${message}`
