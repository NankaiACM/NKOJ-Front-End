<template>
  ...
  <tr class="line" v-for="(it, id) in status" :key="id">
    ...
    <template v-for="(ht, ky, hd) in it">
      <td class="item" :key="hd" v-if="ky !== 'solution_id'">
        {{ht}}
      </td>
    </template>
  </tr>
...
export default {
  ...
  methods: {
    async updatestatus () {
      const newstatus = await contestStatus(this.$http, this.contestid, this.status[0] ? this.status[0].solution_id : 0, this.o.startime, this.o.endtime)
  ...
  beforeDestroy () {
    for (const it of this.pool) clearInterval(it)
  }
}
</script>
<style scoped>
