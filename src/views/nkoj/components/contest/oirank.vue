<template>
  <div id="contest-rank-container-out" class="container-fluid">
    <div id="contest-rank-container-in" class="container-fluid">
      ...
      <div class="person-rank-container" v-for="(person, index) in computedPersons" :key="index">
...
export default {
  ...
  computed: {
    computedPersons () {
      return this.persons.filter((item, index) => isNaN(this.limit) ? true : (index < this.limit))
    }
  },
  methods: {
    toUser (nickname) {
    ...
    tt () {
      return { color: '#' + (~~(Math.random() * 16777215)).toString(16), opacity: 0.6 }
    },
    getProblems () {
