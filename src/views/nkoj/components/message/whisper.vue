...
export default {
  ...
  methods: {
    ...
    clo: function (id) {
      this.$http.get(window.noPointHost + '/api/message/delete/' + id)
        .then((res) => {
          if (res.body.code === 0) {
            this.pool[this.active] = undefined
            this.fetchWords(this.active)
          }
    ...
    report: function (id) {
      this.$http.get(window.noPointHost + '/api/message/report/' + id)
        .then((res) => {
          this.$store.commit('setNotify', {
            title: '举报私信',
            message: res.body.message
    ...
    fetchWords: function (id) {
      if (this.pool[id]) {
        this.words = this.pool[id]
        return -1
      }
      this.$http.get(window.noPointHost + '/api/message/' + id)
        .then((res) => {
          if (res.body.code === 0) {
            this.pool[id] = res.body.data
            this.words = this.pool[id]
          }
        })
    },
    ...
    send: function () {
      ...
      this.$http.post(window.noPointHost + '/api/message/' + this.active, {
        message: this.input
      }).then((res) => {
        console.log(res)
        if (res.body.code === 0) {
          console.log('send ok')
          const newword = { content: this.input }
          console.log(this.words)
          this.pool[this.active].push(newword)
          // this.words.push(newword)  // js 引用
          this.input = ''
        } else {
          this.$store.commit('setNotify', {
            title: '发送私信失败',
