...
export default {
  ...
  methods: {
    withdraw: function (id) {
      ...
      this.$http.get(window.noPointHost + '/api/admin/message/withdraw/' + id)
        .then((res) => {
          if (res.body.code === 0) {
          ...
          fatalcb(this, JSON.stringify(res.body))
        }, (e) => {
          fatalcb(this, JSON.stringify(e))
        })
    },
    upload: function () {
      this.$http.post(window.noPointHost + '/api/admin/message/all', {
        title: this.newpost.title,
        message: this.newpost.content
      }).then((res) => {
        if (res.body.code === 0) {
          this.$store.commit('setNotify', {
            title: '群发新通知成功',
