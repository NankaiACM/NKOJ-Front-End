...
export default {
  ...
  methods: {
    ...
    addUser () {
      if (this.accountNum === 0) return
      this.setColdTime()
      const nameList = this.nameListRaw.replace(' ', '').replace(/[(\r\n)\r\n]+/, '')
        .split(/,|，/).filter(function (item) { return item !== '' })
      console.log(nameList)
      if (nameList.length !== parseInt(this.accountNum) || this.nameListRaw === '') {
        this.message = `Error: Add ${this.accountNum} users, but ${nameList.length} names are given`
        this.isRes = true
        return
      }
      ...
      this.$http.post(window.noPointHost + '/api/admin/user/addmulti/', { nameList: nameList, cid: parseInt(this.contestID), num: parseInt(this.accountNum) }).then(
        res => {
          console.log(JSON.stringify(res))
          this.message = `code: ${res.body.code}, message: ${res.body.message} ` + JSON.stringify(res.body.error)
          const resMsgList = []
