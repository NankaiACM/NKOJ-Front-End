...
export default {
  ...
  methods: {
    ...
    getStandings () {
      this.setColdTime()
      const nameList = this.nameListRaw === '' ? [] : this.nameListRaw.replace(' ', '').split(/[(\r\n)\r\n]+/)
      console.log(nameList)
      for (let i = 0; i < nameList.length; i++) {
        const p = nameList[i].split(/,|，/)
        if (p.length !== 2 || p[0] === '' || p[1] === '') {
          this.message = '错误：打星选手输入格式有误，请不要输入多余的空行'
      ...
      this.$http.post(
        ...
        { nameList: nameList, cid: parseInt(this.contestID) }
      ).then(
        res => {
          console.log(res)
          this.message = `code: ${res.body.code}, message: ${res.body.message} ` + JSON.stringify(res.body.error)
          this.standingsURL = 'data:text/json;charset=UTF-8,' + encodeURIComponent(JSON.stringify(res.body.data))
        },
        err => {
