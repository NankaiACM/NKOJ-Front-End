...
export default {
  ...
  methods: {
    ...
    rejudge: function (solutionId) {
      this.$http.get(window.noPointHost + '/api/judge/rejudge/' + solutionId)
        .then((res) => {
          this.notify.mptdex = -1
    ...
    infiniteHandler: function ($state) {
      ...
      if (this.statusList.length === 0) {
        ...
        this.$http.get(this.apiUrl + '?' + (this.$route.params.querryString ? this.$route.params.querryString : ''))
          .then((res) => {
            if (!res.body.data) {
            ...
            this.statusList = res.body.data
            this.maxId = this.statusList[0].solution_id
            this.minId = this.statusList[this.statusList.length - 1].solution_id
            if ($state.loaded) $state.loaded()
          }, (/* e */) => {
      ...
      const from = this.statusList.length
      const limit = this.filter.limit
      console.log('向魔法机请求更久远的数据')
      this.$http.get(this.apiUrl + '/' + from + '/' + limit + '?' + (this.$route.params.querryString ? this.$route.params.querryString : ''))
        .then((res) => {
          let tmp = this.statusList
          if (!res.body.data) {
          ...
          if (min === this.minId) if ($state.complete) $state.complete()
          this.removeHiger(res.body.data)
          this.minId = min
    ...
    hook: function () {
      if (this.statusList.length === 0) return -1
      const till = this.statusList[0].solution_id
      this.$http.get(this.apiUrl + '/' + till + '?' + (this.$route.params.querryString ? this.$route.params.querryString : ''))
        .then((res) => {
          if (!res.body.data) {
            console.log('无返回')
            return 0
          }
          this.removeLower(res.body.data)
          if (res.body.data.length === 0) {
            console.log('没有新的更新')
            return 0
          }
          this.statusList = res.body.data.concat(this.statusList)
          this.maxId = this.statusList[0].solution_id
          console.log('魔法状态更新')
        }, (e) => {
  ...
  mounted: function () {
    this.$nextTick(() => {
      this.pool.push(window.setInterval(this.hook, 10000))
    })
  },
  beforeDestroy: function () {
    console.log('清理魔法池')
    for (const i in this.pool) {
      window.clearInterval(this.pool[i])
    }
  },
  components: {
    InfiniteLoading
