          ...
          <ul>
            <li v-for="(problem,index) in problems" :key="index">
              <problem-list :problem-index="index" :problem-name="problem.title"
                            ...
                            :url="'/nkpc.html#/coding/' + contestid + '/' + problem.problem_id"/>
            </li>
          </ul>
        </div>
      </div>
...
import { myStatus, contestStatus as contestStatusFunc } from './components/contest/virtualApi.js'
...
export default {
  ...
  methods: {
    async test () {
      ...
      const ctNewStatus = await contestStatusFunc(this.$http, this.contestid, this.status[0] ? this.status[0].solution_id : 0)
      this.status.concat(ctNewStatus)
    },
    async statusUpdata () {
      const newStatus = await contestStatusFunc(this.$http, this.contestid, this.status[0] ? this.status[0].solution_id : 0, this.startTime, this.endTime)
    ...
    handleScroll () {
      ...
      const band = document.querySelectorAll('div.band-with-80padding')
    ...
    changeProblemListHeight () {
      const problemList = document.querySelectorAll('.problem-list')
      let maxnum = 0
    ...
    initDatas: function () {
      console.log('in initDatas', this.contestid = this.$route.params.contestid.toString())
      this.$http.get(`${noPointHost}/api/contest/${this.contestid}`)
        .then((res) => {
          res = res.body.data
          const [f, t] = JSON.parse(res.during)
          this.startTime = f
          this.endTime = t
          this.contestTitle = res.title
          this.navbarItems = []
          this.MDtext = new Array(1)
          this.MDtext[0] = res.description
          this.rule = res.rule
          this.problems = res.problems.sort(function (l, r) {
            return l.problem_id - r.problem_id
          })
          if (pidorders[this.contestid]) {
            const mp = pidorders[this.contestid]
            this.problems = res.problems.sort(function (l, r) {
    ...
    registerAttempt: function () {
      if (this.userData === undefined) {
        ...
        this.$http
          .get(
            `${noPointHost}/api/user/contest/register/` + this.contestid,
            {
              crossDomain: true,
              xhrFields: { withCredentials: true },
              timeout: '8000',
          ...
          .then(
            res => {
              if (res.body.code === 0) {
                this.dialogMessage = '报名成功！'
              } else if (res.body.code === 3) {
                this.dialogMessage = '您已经报名过了'
              } else {
                this.dialogMessage = '操作非法！'
              }
            },
            (/* res */) => {
              // todo
  ...
  beforeDestroy: function () {
    ...
    for (const it of this.tick) window.clearInterval(it)
  },
  computed: {
    countDown: function () {
